...
def create_princ():
    ...
    return root, notebook, input_files, output_dir, bold_font, background

def create_files_tab(notebook, input_files, output_dir, background):
    #### ONGLET FICHIERS
    ...
    select_button = tk.Button(frame_buttons, text="Vidéos...", width=18,
        command=lambda: libtools.select_files(input_files, files_list), bg="orange", fg="white")
    select_button.pack(side="left")

    remove_button = tk.Button(frame_buttons, text="Supprimer sélection", width=18,
        command=lambda: libtools.remove_selected(input_files, files_list), bg="#d9534f", fg="white")
    remove_button.pack(side="left", padx=5)

    clear_button = tk.Button(frame_buttons, text="Tout effacer", width=14,
        command=lambda: libtools.clear_all(input_files, files_list), bg="#6c757d", fg="white")
    ...
    output_button = tk.Button(frame_output, text="Destination...", width=20,
                              command=lambda: libtools.select_output_dir(output_dir), bg="orange", fg="white")
...
def create_options_tab(notebook, bold_font, background):
...
def create_help_tab(notebook, bold_font, background):
    #### ONGLET AIDE
    ...
    tk.Label(frame_help, text=f"• CUDA {'est disponible' if libtools.check_cuda() else 'n\'est pas disponible'} sur ce système.", bg=background).pack(anchor="w")